...
main    MOV R0, #(PORTF_BIT)
        BL Enable_port                 ; habilita a porta n

        LDR R0, =GPIO_PORTF_BASE        ; carrega o valor (endereço de memoria) GPIO_PORTN_BASE em R0
        MOV R1, #000010001b             ; coloca o valor de 0011b em binario no registrador R1    
        BL Enable_GPIO_output           ; esse valor 0011b indica quais bits da porta n poderam ser acessados
                                        ; os bits 0 e 4 são os bits dos leds da placa para a porta F
        
        LDR R0, =GPIO_PORTF_BASE        ; linha repetida
        MOV R1, #000010001b             ; máscara dos LEDs D1 e D2 -> indica que apenas os bits 0x0011b do registrador poderao ser usados
...
loop:   BL Digital_write                ; aciona LEDs D1 e D2
        ...
        EOR R2, R2, #10001b                ; OU exclusivo -> 0||1 = 1, 0||0=0, 1||1=0
        B loop


Enable_port:                            ; -> habilita o GPIO da porta F
